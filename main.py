...
def parse_dataset():
    ...
    try:
        ...
        return collections.namedtuple('Engine', ['page_text_corpus', 'page_text_corpus_embeddings', 'page_numbers_list', 'chapter_number_list'])(
            page_text_corpus, page_text_corpus_embeddings, page_numbers_list, chapter_number_list)
...
def get_query_embedding_openai(prompt):
    ...
    """
    try:
        response = openai.Embedding.create(model=EMBEDDINGS_MODEL, input=prompt)
...
def run_flask_app():
    ...
    def ask():
        try:
            ...
            return {"prompt": combined_prompt, "answer": answer}
        except Exception as e:
